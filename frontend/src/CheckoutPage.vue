...
// ✅ แก้ไข Path: ลองใช้จุดเดียวถ้า CheckoutPage.vue อยู่ระดับเดียวกับโฟลเดอร์ services
...
const getImageUrl = (item) => {
  const imagePath = item.image || item.img || item.imageUrl || item.thumbnail;
  if (!imagePath) return 'https://via.placeholder.com/80';
  
  if (imagePath.startsWith('/')) {
    const baseUrl = window.location.hostname === 'localhost' 
      ? "http://localhost:3000" 
      : "https://appdev-pulo.onrender.com";
        ...
        <aside class="summary-panel">
          <div class="summary-card">
            ...
            <div v-for="(item, index) in props.cartItems" :key="index" class="summary-item">
              <img :src="getImageUrl(item)" alt="Product" class="summary-image" />
