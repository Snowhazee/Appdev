...
// ✅ เปลี่ยนมาใช้ api.js แทน axios ตรงๆ
...
async function fetchData() {
  ...
  loading.value = true
  try {
    // ✅ ใช้ api.get แทน axios (ไม่ต้องใส่ baseURL และ headers เองแล้ว)
    const pRes = await api.get('/users/profile')
    user.value = pRes.data
    
    const oRes = await api.get('/orders/myorders')
    orders.value = oRes.data
    
    if (orders.value.length > 0) currentTab.value = 'orders'
  } catch (err) {
    console.error("Fetch Error:", err.response?.data || err.message)
    // ถ้า Token หมดอายุ หรือดึงข้อมูลไม่ได้ ให้ Logout ออกไปเลย
    if (err.response?.status === 401) emit('logout')
  } finally {
    loading.value = false
  }
}
        ...
        <div v-else class="acc-inner-content">
          ...
          <div v-if="currentTab === 'profile'" class="profile-data">
            <div class="field-box">
              ...
              <div class="value-text">{{ user.name || user.username || 'No Name' }}</div>
            </div>
            <div class="field-box">
            ...
            <div class="field-box">
              ...
              <div><span class="role-badge">{{ user.role || 'User' }}</span></div>
            </div>
          </div>
            ...
            <div v-else class="order-scroll-area">
              <div v-for="order in orders" :key="order._id" class="order-card-mini">
                <div style="display:flex; justify-content:space-between; font-weight:bold;">
                  ...
                  <span>{{ (order.totalPrice || 0).toLocaleString() }} บาท</span>
                </div>
...
.acc-white-card { background: #fff; padding: 25px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); min-height: 300px; }
.field-box { margin-bottom: 20px; text-align: center; }
.field-box label { display: block; font-size: 11px; color: #BBB; font-weight: bold; margin-bottom: 5px; }
.value-text { font-size: 18px; font-weight: bold; color: #333; }
.role-badge { background: #9EDAFF; padding: 2px 10px; border-radius: 15px; font-size: 11px; color: #fff; text-transform: uppercase; }
.btn-logout { width: 100%; padding: 12px; background: #FF5C5C; color: #fff; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; margin-top: 25px; }
.btn-logout:hover { background: #ff4444; }
.order-card-mini { padding: 12px; border: 1px solid #EEE; border-radius: 6px; margin-bottom: 10px; text-align: left; }
.order-scroll-area { max-height: 300px; overflow-y: auto; padding-right: 5px; }
.empty-orders { text-align: center; color: #999; padding: 40px 20px; }
</style>